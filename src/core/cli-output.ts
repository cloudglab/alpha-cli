...
export function printHelp(role: Role, commands: CliCommandDefinition[]): void {
  ...
  process.stdout.write([
    ...
    '',
    '用法：',
    '  alpha [--role full|ci|deploy|iter|rbac|file] [--output compact|normal|verbose] <command> [options]',
    '  alpha [--role=full|ci|deploy|iter|rbac|file] [--output=compact|normal|verbose] <command> [options]',
    '',
    '快速开始：',
    ...
    '',
    'AI 友好提示：',
    '  - 默认 --output compact 会裁剪大列表与长文本，AI 推理时优先看 normal/verbose 模式',
    ...
    '',
    '输出模式：',
    '  compact  默认紧凑输出，适合脚本与大列表',
    '  normal   保留常用 meta 字段（command、method、path、group、total 等）',
    '  verbose  返回完整原始 JSON',
    '',
    '更多帮助：',
...
export function getBuiltinCommandHelp(commandName: string): string | undefined {
  ...
  const builtins: Record<string, string> = {
    ...
    install: [
      ...
      '',
      '说明：',
      '  通过 npm 全局安装最新 alpha-cli，并在成功后打印 ASCII Banner。',
      '',
    ].join('\n'),
    update: [
      'alpha update',
      ...
      '',
      '说明：',
      '  更新全局 alpha-cli 到最新版本，并在成功后打印 ASCII Banner。',
      '',
    ].join('\n'),
    upgrade: [
      'alpha update',
...
function describeBuiltin(commandName: string): string {
  const descriptions: Record<string, string> = {
    ...
    list: '列出当前 role 可用命令',
    version: '查看 CLI 版本',
    install: '安装全局 CLI 并打印 Banner',
    update: '更新全局 CLI 并打印 Banner',
    upgrade: 'update 的别名',
  };
